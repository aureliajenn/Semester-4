...
public class WindowThread extends Thread {
    ...
    @Override
    public void run() {
        // TODO: hitung window sum untuk indeks [startWin, endWin) lalu simpan di array
        // sums
        if (startWin < endWin) {
            long sum = 0;
            for (int j = startWin; j < startWin + windowSize; j++) {
                sum += data[j];
            }
            sums[startWin] = (int) sum;

            for (int i = startWin + 1; i < endWin; i++) {
